<script setup lang="ts">
  ...
  async function deleteByIds(row, index) {
    try {
      ...
      try {
        ...
        await brandApi.deleteByIds(ids)
        ElMessage({
          message: '删除/批量删除成功',
          type: 'success',
          duration: 1000,
  ...
  async function updateById(row, index, done, loading) {
    try {
      loading()
      await brandApi.updateById(data.form.id, data.form)
      done()
      ElMessage({
        message: '修改成功',
        type: 'success',
  ...
  async function getList(page, done) {
    try {
      ...
      let res = await brandApi.getListByPageAndParam(Object.assign(newPage, data.search))
